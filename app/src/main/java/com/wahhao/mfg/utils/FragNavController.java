...
public class FragNavController {
    ...
    @Nullable
    private RottfragmentListener mRottfragmentListener;
    @Nullable
    ...
    private FragNavController(Builder builder, @Nullable Bundle savedInstanceState) {
        ...
        mRottfragmentListener = builder.mRottfragmentListener;
        ...
        if (!restoreFromBundle(savedInstanceState, builder.mRottfragments)) {

            for (int i = 0; i < builder.mNumberOfTabs; i++) {
                Stack<Fragment> stack = new Stack<>();
                if (builder.mRottfragments != null) {
                    stack.add(builder.mRottfragments.get(i));
                }
    ...
    public void switchTab(@TabIndex int index, @Nullable FragNavTransactionOptions transactionOptions) throws IndexOutOfBoundsException {
        ...
        if (index >= mFragmentStacks.size()) {
            throw new IndexOutOfBoundsException("Can't switch to a tab that hasn't been initialized, " +
                    ...
                    ". Make sure to create all of the tabs you need in the Constructor or provide a way for them to be created via RottfragmentListener.");
        }
        if (mSelectedTabIndex != index) {
            ...
            if (index == NO_TAB) {
                ...
                if (fragment != null) {
                    ...
                    fragment = getRottfragment(mSelectedTabIndex);
    ...
    public void popFragments(int popDepth, @Nullable FragNavTransactionOptions transactionOptions) throws UnsupportedOperationException {
        if (isRottfragment()) {
            throw new UnsupportedOperationException(
                    "You can not popFragment the rottfragment. If you need to change this fragment, use replaceFragment(fragment)");
        } else if (popDepth < 1) {
    ...
    public void replaceStackOnboard(@Nullable FragNavTransactionOptions transactionOptions) {
        if (mSelectedTabIndex == NO_TAB) {
            return;
        }
        //Grab Current stack
    ...
    private void initialize(@TabIndex int index) {
        ...
        Fragment fragment = getRottfragment(index);
        ft.add(mContainerId, fragment, generateTag(fragment));
        ft.commit();
     ...
     * @throws IllegalStateException This will be thrown if we can't find a rottfragment for this index. Either because you didn't provide it in the
     *                               constructor, or because your RottfragmentListener.getRottfragment(index) isn't returning a fragment for this index.
     */
    @NonNull
    @CheckResult
    private Fragment getRottfragment(int index) throws IllegalStateException {
        Fragment fragment = null;
        if (!mFragmentStacks.get(index).isEmpty()) {
            fragment = mFragmentStacks.get(index).peek();
        } else if (mRottfragmentListener != null) {
            fragment = mRottfragmentListener.getRottfragment(index);
        ...
        if (fragment == null) {
            throw new IllegalStateException("Either you haven't past in a fragment at this index in your constructor, or you haven't " +
                    "provided a way to create it while via your RottfragmentListener.getRottfragment(index)");
        }
     ...
     * @param rottfragments      List of root fragments from which to initialize empty stacks. If null, pull fragments from RottfragmentListener.
     * @return true if successful, false if not
     */
    private boolean restoreFromBundle(@Nullable Bundle savedInstanceState, @Nullable List<Fragment> rottfragments) {
            ...
            for (int x = 0; x < stackArrays.length(); x++) {
                ...
                if (stackArray.length() == 1) {
                    ...
                    if (tag == null || "null".equalsIgnoreCase(tag)) {
                        if (rottfragments != null) {
                            fragment = rottfragments.get(x);
                        } else {
                            fragment = getRottfragment(x);
                        }
    ...
    public interface RottfragmentListener {
        /**
         ...
         */
        Fragment getRottfragment(int index);
    }
    ...
    public static final class Builder {
        ...
        private List<Fragment> mRottfragments;
        ...
        /**
         * @param selectedTabIndex The initial tab index to be used must be in range of rottfragments size
         */
        public Builder selectedTabIndex(@TabIndex int selectedTabIndex) {
            mSelectedTabIndex = selectedTabIndex;
            if (mRottfragments != null && mSelectedTabIndex > mNumberOfTabs) {
                throw new IndexOutOfBoundsException("Starting index cannot be larger than the number of stacks");
            }
            return this;
        }
        ...
        /**
         * @param rottfragment A single root fragment. This library can still be helpful when managing a single stack of fragments
         */
        public Builder rottfragment(Fragment rottfragment) {
            mRottfragments = new ArrayList<>(1);
            mRottfragments.add(rottfragment);
            mNumberOfTabs = 1;
            return rottfragments(mRottfragments);
        }

        /**
         * @param rottfragments a list of root fragments. root Fragments are the root fragments that exist on any tab structure. If only one fragment is sent in, fragnav will still manage
         *                      transactions
         */
        public Builder rottfragments(@NonNull List<Fragment> rottfragments) {
            mRottfragments = rottfragments;
            mNumberOfTabs = rottfragments.size();
        ...
        /**
         * @param rottfragmentListener a listener that allows for dynamically creating root fragments
         * @param numberOfTabs         the number of tabs that will be switched between
         */
        public Builder rottfragmentListener(RottfragmentListener rottfragmentListener, int numberOfTabs) {
            mRottfragmentListener = rottfragmentListener;
        ...
        public FragNavController build() {
            if (mRottfragmentListener == null && mRottfragments == null) {
